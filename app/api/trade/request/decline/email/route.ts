...
export async function POST(req: Request) {
    try {
        ...
        const send = await transport.sendMail({
            ...
            html: declineemail(`${tradeinfo?.Book_Trade_book_idToBook.title}`, `${tradeinfo?.Book_Trade_req_book_idToBook.title}`, `${tradeinfo?.User_Trade_owner_idToUser.username}`, `${tradeinfo?.User.username}`, `${tradeinfo.Book_Trade_book_idToBook.picture[0]}`,  `${logo}`)
        })
